...
<div class="InputContainer">  
  ...
  <React.Fragment>
    ...
    <TableContainer component={Paper} sx={{ maxWidth: 900, margin: "auto", mt: 5 }}>
        ...
        <TableBody>
          {data.map((e) => (
            <TableRow key={e.id} hover>
              <TableCell><input onClick={()=>ch(e)} checked={e.status} type="checkbox"/></TableCell>
              <TableCell>{e.id}</TableCell>
              <TableCell>
                <div className="flex items-center gap-[10px]">
                <img className="w-[40px] rounded-[50%]" src={e.img} alt="" />
                <div className="flex flex-col">
                <h1 className="text-[18px]">{e.name}</h1>
                <p>{e.job}</p>
                </div>
                </div>
                </TableCell>
              <TableCell>{e.age}</TableCell>
              <TableCell sx={e.status?"color:green":"color:red"} >{e.status?"Active":"Inactive"}</TableCell>
              <TableCell><Button onClick={() => { xx(e);editopen();}} variant="contained" size="small">EDITE</Button><IconButton onClick={()=>del(e.id)} aria-label="delete" size="large">
        <DeleteIcon fontSize="inherit" />
      </IconButton>
      <IconButton onClick={()=>infoo(e.id)}>
      ...
      <Dialog open={infoopen} onClose={infclose}>
        <DialogTitle><div className="w-[400px] p-[10px] text-center">
          <img className="w-[200px] rounded-[50%]" style={{margin:"auto"}} src={info?.img} alt="" />
          <h1 className="text-[18px]" style={{margin:"10px 0px"}} >Name:{info?.name}</h1>
          <p className="text-[14px]">Job:{info?.job}</p>
          <h1>Age:{info?.age}</h1>
        ...
        <DialogActions>
          <Button onClick={infclose}>Cancel</Button>
        </DialogActions>
      </Dialog>
