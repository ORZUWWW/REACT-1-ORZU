  ...
  {data.map((e, i) => {
    return (
      ...
      >
        ...
        <p className="text-sm text-gray-500">О
          ID: {e.id}
        </p>

        <input onChange={()=>ch(e.id)} type="checkbox" checked={e.isCompleted} />

        <div className='border p-[10px] rounded-2xl'>
          {
            e.images.map((img)=>{
              return <img className='w-[300px]' src={`${image}/${img.imageName}`} alt="" />
            })
          }
        </div>
        ...
        <div className="mt-4 gap-1 flex justify-end">
          ...
          <button className="text-sm bg-blue-600 text-white px-3 py-1 rounded-lg hover:bg-blue-600 transition">
            EDITE
          </button>
        </div>
      </div>
